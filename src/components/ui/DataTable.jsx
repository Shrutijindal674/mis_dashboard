...
export default function DataTable({ columns, rows, maxHeight = 520, onRowClick, footerRight }) {
  return (
    <div className="overflow-auto rounded-2xl border border-zinc-200" style={{ maxHeight }}>
      <table className="w-full border-collapse text-sm">
        <thead className="sticky top-0" style={{ background: "#eef4fb" }}>
          <tr>
            {columns.map((c) => (
              <th
                key={c.key}
        ...
        <tbody>
          {rows.map((r, idx) => (
            <tr
              ...
              style={{ background: idx % 2 === 0 ? "#ffffff" : "#eef5ff" }}
